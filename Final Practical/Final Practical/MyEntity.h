...
class MyEntity
{
	...
	//holds the shortest path to the next node
	int shortestPath = 100000000000;
	
public:
	//Holds the connections to this entity 
	std::vector<MyEntity*> connections;

	//holds the previous node path from this node.
	MyEntity* previousNode = nullptr;

	//holds the path values to the connections
	...
	*/
	...
	void SetPositon(vector3 vPosition);

	vector3 GetPosition(void);

	void SetShortestPath(int vShortestPath);

	int GetShortestPath(void);

	void SetDistanceLength(int vDistance);
	
	int GetDistanceLength(void);
